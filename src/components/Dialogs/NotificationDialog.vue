  ...
  >
    <q-card>
      <q-card-section class="row items-center q-pb-none justify-between">
        ...
        <q-btn icon="close" flat round dense class="q-ml-sm bg-block" size="sm" text-color="negative" v-close-popup />
      </q-card-section>
      <q-card-section class="dialog-content">
        <div v-if="invites.length > 0" class="flex column q-gap-sm">
          <div
            v-for="invite in invites" :key="invite.documentId"
            ...
            <div class="notification-item-content flex column items-start q-gap-sm">
              <div class="flex items-center justify-between q-gap-sm full-width q-mb-sm">
                <q-badge color="primary" class="notification-item-sender text-caption border-radius-sm">Invitation</q-badge>
                <div class="notification-item-time text-caption bg-block border-radius-sm q-px-sm q-py-xs text-grey-6">Created at: {{ formatDate(invite.createdAt) }}</div>
              </div>
        ...
        <div v-else class="flex justify-center items-center q-h-full q-mt-lg full-width column q-gap-sm">
